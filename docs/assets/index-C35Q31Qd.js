...
7/ 6- X 81 9/ X 7- X X X90`},{name:"Clutch Performance",description:"Strong finish with strikes in the 10th",score:"7/ 8/ 81 9- 72 X 9/ 8- X XXX"},{name:"All Spares Game",description:"Consistent spare shooting - no strikes, no open frames",score:"9/ 8/ 7/ 6/ 5/ 4/ 3/ 2/ 1/ 9/9"}],be=document.querySelector("#app");if(!be)throw new Error("Failed to find app container");be.innerHTML=`
  ...
  <footer class="version">
    <p>Build: 2025-10-18 07:39:04 CT</p>
  </footer>
...
`;const S=document.querySelector("#scores-input"),j=document.querySelector("#submit"),ye=document.querySelector("#clear-btn"),N=document.querySelector("#example-btn"),q=document.querySelector("#example-dropdown"),C=document.querySelector("#feedback"),$e=document.querySelector("#save-btn"),Se=document.querySelector("#saved-games-btn"),xe=document.querySelector("#saved-count"),B=document.querySelector("#save-modal-overlay"),we=document.querySelector("#save-form"),V=document.querySelector("#save-description"),ae=document.querySelector("#save-league"),ke=document.querySelector("#league-list"),J=document.querySelector("#save-date"),Le=document.querySelector("#save-cancel-btn"),U=document.querySelector("#saved-games-sidebar"),Z=document.querySelector("#sidebar-overlay"),Me=document.querySelector("#sidebar-close-btn"),Q=document.querySelector("#search-saved-games"),Xe=document.querySelector("#export-btn"),Ae=document.querySelector("#import-btn"),R=document.querySelector("#import-file-input"),Ce=document.querySelector("#clear-all-btn"),O=document.querySelector("#saved-games-list"),Ee=document.querySelector("#sidebar-saved-count");if(!S||!j||!ye||!N||!q||!C||!$e||!Se||!xe||!B||!we||!V||!ae||!ke||!J||!Le||!U||!Z||!Me||!Q||!Xe||!Ae||!R||!Ce||!O||!Ee)throw new Error("Failed to initialise UI elements");ye.addEventListener("click",()=>{S.value="",C.innerHTML="",S.focus()});let I=!1;function Qe(){I=!I,q.classList.toggle("show",I),N.setAttribute("aria-expanded",I.toString()),q.setAttribute("aria-hidden",(!I).toString())}function ie(){I=!1,q.classList.remove("show"),N.setAttribute("aria-expanded","false"),q.setAttribute("aria-hidden","true")}N.addEventListener("click",e=>{e.stopPropagation(),Qe()});const Ie=q.querySelectorAll(".dropdown-item");Ie.forEach(e=>{e.addEventListener("click",o=>{o.stopPropagation();const t=parseInt(e.getAttribute("data-example-index")||"0",10);S.value=ve[t].score,ie(),S.focus()})});document.addEventListener("click",e=>{const o=e.target;I&&!N.contains(o)&&!q.contains(o)&&ie()});q.addEventListener("keydown",e=>{if(e.key==="ArrowDown"||e.key==="ArrowUp"){e.preventDefault();const o=Array.from(Ie),t=o.indexOf(document.activeElement);let a;e.key==="ArrowDown"?a=t<o.length-1?t+1:0:a=t>0?t-1:o.length-1,o[a]?.focus()}});let oe=0;function qe(){if(Math.random()<Ze){const o=Math.floor(Math.random()*me.length);j.textContent=me[o];return}j.textContent=ue[oe],oe=(oe+1)%ue.length}qe();setInterval(qe,3e4);let Te="";function ce(){if(!S.value.trim()){ne("Please provide at least one game.",1,1);return}const e=S.value.replace(/\r/g,"").split(`
`),o=[];for(let t=0;t<e.length;t+=1){const a=e[t];if(!a.trim()){ne(`Game ${t+1} is empty. Each line must contain exactly ten frames.`,t+1,1);return}const r=he(a);if(r.kind==="error"){nt(r,t,e);return}const s=z(r.frames),n=je(r.frames);o.push({frames:r.frames,score:s,stats:n})}Te=S.value,pt(o)}j.addEventListener("click",ce);S.addEventListener("keydown",e=>{e.key==="Enter"&&(e.ctrlKey||e.metaKey)&&(e.preventDefault(),ce())});document.addEventListener("keydown",e=>{if(e.key==="Escape"){if(U.classList.contains("show")){te();return}if(B.classList.contains("show")){ee();return}if(I){ie(),N.focus();return}C.innerHTML&&(C.innerHTML="",S.focus())}});function Y(){const o=H().length;xe.innerHTML=o>0?`&nbsp;(${o})`:"",Ee.innerHTML=o>0?`&nbsp;(${o})`:""}function et(){if(!S.value.trim()){L("Please enter some scores first");return}const e=new Date().toISOString().split("T")[0];J.value=e,V.value="",ae.value="";const o=_e();ke.innerHTML=o.map(t=>`<option value="${t}">`).join(""),B.classList.add("show"),V.focus()}function ee(){B.classList.remove("show")}function tt(){Q.value="",P(),U.classList.add("show"),Z.classList.add("show")}function te(){U.classList.remove("show"),Z.classList.remove("show")}function P(){const e=Q.value.trim().toLowerCase();let o=H();if(e&&(o=o.filter(t=>{const a=(t.description||"").toLowerCase(),r=(t.league||"").toLowerCase();return a.includes(e)||r.includes(e)})),o.length===0){O.innerHTML=e?'<p class="empty-state">No games match your search.</p>':'<p class="empty-state">No saved games yet. Save your first game!</p>';return}O.innerHTML=o.map(t=>{const a=t.gameCount===1?"1 game":`${t.gameCount} games`,r=t.totalScore!==void 0?`🎯 ${t.totalScore}`:"⚠️ Invalid",s=t.description||"(No description)",n=t.league?`🏆 ${t.league}`:"";return`
...
Combinations: ${u.count.toLocaleString()}</title>
  ...
  `,y="",b=Math.min(10,Math.ceil((f-m)/20)),v=b===0?`
      <line x1="${n.left}" y1="${n.top+i}" x2="${n.left}" y2="${n.top+i+5}" stroke="#94a3b8" stroke-width="1" />
      <text x="${n.left}" y="${n.top+i+20}" text-anchor="middle" font-size="11" fill="#94a3b8">${m}</text>
    `:Array.from({length:b+1},(u,g)=>{const G=Math.round(m+(f-m)/b*g),E=n.left+g*c/b;return`
        <line x1="${E}" y1="${n.top+i}" x2="${E}" y2="${n.top+i+5}" stroke="#94a3b8" stroke-width="1" />
        <text x="${E}" y="${n.top+i+20}" text-anchor="middle" font-size="11" fill="#94a3b8">${G}</text>
      `}).join("");return`
    <svg viewBox="0 0 ${r} ${s}" class="histogram">
      <rect x="0" y="0" width="${r}" height="${s}" fill="rgba(15, 23, 42, 0.5)" />
      ${T}
  ...
  `}function ft(e){const o=_(e),t=He(e),a=3,r=-3,s=t.filter(l=>l.positionBenefit>=a).sort((l,d)=>d.positionBenefit-l.positionBenefit),n=t.filter(l=>l.positionBenefit<=r).sort((l,d)=>l.positionBenefit-d.positionBenefit);function c(){return`
      <div class="complete-scorecard">
        <div class="scorecard-row">
