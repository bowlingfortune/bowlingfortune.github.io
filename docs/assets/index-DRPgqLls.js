(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))a(r);new MutationObserver(r=>{for(const s of r)if(s.type==="childList")for(const n of s.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&a(n)}).observe(document,{childList:!0,subtree:!0});function t(r){const s={};return r.integrity&&(s.integrity=r.integrity),r.referrerPolicy&&(s.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?s.credentials="include":r.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function a(r){if(r.ep)return;r.ep=!0;const s=t(r);fetch(r.href,s)}})();const Je=new Set([" ","	",",",";"]),fe=new Set("0123456789-".split("")),We=new Set("0123456789-X/".split(""));function ke(e){return Je.has(e)}function F(e){const{line:o}=e;for(;e.index<o.length&&ke(o[e.index]);)e.index+=1;if(e.index>=o.length)return null;const t=e.index+1,a=o[e.index].toUpperCase();return e.index+=1,{char:a,column:t}}function D(e){if(e==="X")return 10;if(e==="-")return 0;const o=Number.parseInt(e,10);if(Number.isNaN(o))throw new Error(`Invalid roll symbol '${e}'`);if(o<0||o>9)throw new Error(`Invalid roll value '${e}'`);return o}function xe(e){return e==="X"||fe.has(e)}function ue(e){return We.has(e)}function M(e,o,t){return{symbol:e,value:o,column:t}}function Le(e){const o=[],t={line:e,index:0},a=()=>{for(;t.index<e.length&&ke(e[t.index]);)t.index+=1};for(let s=0;s<9;s+=1){if(a(),t.index>=e.length)return{kind:"error",message:`Expected frame ${s+1}, but the line ended early`,column:e.length+1};const n=F(t);if(!n)return{kind:"error",message:`Expected frame ${s+1}, but found nothing`,column:e.length+1};const{char:c,column:i}=n;if(!xe(c))return{kind:"error",message:`Invalid roll '${c}' in frame ${s+1}`,column:i};if(c==="X"){o.push({rolls:[M("X",10,i)],isStrike:!0,isSpare:!1});continue}const m=D(c),f=F(t);if(!f)return{kind:"error",message:`Frame ${s+1} is missing a second roll`,column:e.length+1};const{char:l,column:d}=f;if(l==="X")return{kind:"error",message:`Strike symbol not allowed in second roll of frame ${s+1}`,column:d};if(l==="/"){if(m>=10)return{kind:"error",message:`Spare in frame ${s+1} requires the first roll to be less than 10`,column:d};const $=10-m;o.push({rolls:[M(c,m,i),M("/",$,d)],isStrike:!1,isSpare:!0});continue}if(!fe.has(l))return{kind:"error",message:`Invalid roll '${l}' in frame ${s+1}`,column:d};const p=D(l);if(m+p>10)return{kind:"error",message:`Pins knocked down exceed 10 in frame ${s+1}`,column:d};o.push({rolls:[M(c,m,i),M(l,p,d)],isStrike:!1,isSpare:!1})}if(a(),t.index>=e.length)return{kind:"error",message:"Frame 10 is missing",column:e.length+1};const r=Ze(t,e);return r.kind==="error"?r:(o.push(r.frame),a(),t.index<e.length?{kind:"error",message:"Too many rolls provided. Expected exactly 10 frames.",column:t.index+1}:{kind:"success",frames:o})}function Ze(e,o){const t=F(e);if(!t)return{kind:"error",message:"Frame 10 is missing",column:o.length+1};const{char:a,column:r}=t;if(!xe(a))return{kind:"error",message:`Invalid roll '${a}' in frame 10`,column:r};if(a==="X")return Qe(e,r);const s=D(a),n=F(e);if(!n)return{kind:"error",message:"Frame 10 is missing a second roll",column:o.length+1};const{char:c,column:i}=n;if(c==="X")return{kind:"error",message:"Strike symbol not allowed as the second roll unless the first was a strike",column:i};if(c==="/"){if(s>=10)return{kind:"error",message:"Spare in frame 10 requires the first roll to be less than 10",column:i};const f=10-s,l=F(e);if(!l)return{kind:"error",message:"A bonus ball is required after a spare in frame 10",column:o.length+1};const{char:d,column:p}=l;if(d==="/")return{kind:"error",message:"Spare symbol cannot be used for the fill ball after a spare",column:p};if(!ue(d))return{kind:"error",message:`Invalid fill ball '${d}' in frame 10`,column:p};const $=d==="X"?10:D(d);return{kind:"success",frame:{rolls:[M(a,s,r),M("/",f,i),M(d,$,p)],isStrike:!1,isSpare:!0}}}if(!fe.has(c))return{kind:"error",message:`Invalid roll '${c}' in frame 10`,column:i};const m=D(c);return s+m>10?{kind:"error",message:"Pins knocked down exceed 10 in frame 10",column:i}:{kind:"success",frame:{rolls:[M(a,s,r),M(c,m,i)],isStrike:!1,isSpare:!1}}}function Qe(e,o){const t=F(e);if(!t)return{kind:"error",message:"A strike in frame 10 requires two additional rolls",column:o};const{char:a,column:r}=t;if(!ue(a)||a==="/")return{kind:"error",message:"Invalid second roll after a strike in frame 10",column:r};let s;a==="X"?s=10:s=D(a);const n=F(e);if(!n)return{kind:"error",message:"A strike in frame 10 requires two additional rolls",column:r};const{char:c,column:i}=n;if(!ue(c))return{kind:"error",message:`Invalid fill ball '${c}' in frame 10`,column:i};let m;if(c==="X")m=10;else if(c==="/"){if(a==="X")return{kind:"error",message:"Spare symbol cannot follow a strike in the second roll of frame 10",column:i};if(s>=10)return{kind:"error",message:"Spare symbol invalid because there are no pins remaining",column:i};m=10-s}else if(m=D(c),a!=="X"&&s+m>10)return{kind:"error",message:"Pins knocked down exceed the remaining pins in frame 10",column:i};return{kind:"success",frame:{rolls:[M("X",10,o),M(a,s,r),M(c,m,i)],isStrike:!0,isSpare:!1}}}function K(e){const o=[],t=[],a=[];for(const n of e){for(const c of n.rolls)o.push(c.value);t.push(n.isStrike),a.push(n.isSpare)}let r=0,s=0;for(let n=0;n<10;n+=1)t[n]?(r+=10+(o[s+1]??0)+(o[s+2]??0),s+=1):a[n]?(r+=10+(o[s+2]??0),s+=2):(r+=(o[s]??0)+(o[s+1]??0),s+=2);return r}function et(e){if(e.length!==10)throw new Error("Expected exactly 10 frames");const o=e.slice(0,9),t=e[9],a=[];function r(s,n){if(n===1){a.push([...s,t]);return}for(let c=0;c<n;c++)r(s,n-1),n%2===0?[s[c],s[n-1]]=[s[n-1],s[c]]:[s[0],s[n-1]]=[s[n-1],s[0]]}return r(o,o.length),a}function tt(e){const o=et(e),t=o.map(h=>K(h)),a=K(e);t.sort((h,S)=>h-S);const r=t[0],s=t[t.length-1],c=t.reduce((h,S)=>h+S,0)/t.length,i=Math.floor(t.length/2),m=t.length%2===0?(t[i-1]+t[i])/2:t[i],f=new Map;for(const h of t)f.set(h,(f.get(h)||0)+1);let l=0;for(const h of f.values())h>l&&(l=h);const d=[];for(const[h,S]of f)S===l&&d.push(h);d.sort((h,S)=>h-S);const p=[];for(const[h,S]of f)p.push({score:h,count:S,frequency:S/t.length});p.sort((h,S)=>h.score-S.score);const $=t.filter(h=>h<=a).length,k=Math.round($/t.length*100*100)/100,w=t.reduce((h,S)=>h+Math.pow(S-c,2),0)/t.length,x=Math.sqrt(w),q=x===0?0:(a-c)/x,E=t.reduce((h,S)=>h+Math.pow((S-c)/x,3),0),T=x===0?0:E/t.length;return{min:r,max:s,mean:Math.round(c*100)/100,median:m,mode:d,permutationCount:o.length,histogram:p,actualPercentile:k,zScore:Math.round(q*100)/100,skewness:Math.round(T*100)/100,standardDeviation:Math.round(x*100)/100}}function J(e){const o=[],t=[],a=[];for(const c of e){for(const i of c.rolls)o.push(i.value);t.push(c.isStrike),a.push(c.isSpare)}const r=[];let s=0,n=0;for(let c=0;c<10;c+=1){const i=e[c];let m=0,f=0,l="";t[c]?(m=10+(o[s+1]??0)+(o[s+2]??0),f=10,c===9?l=i.rolls.map(d=>d.symbol).join(" "):l="X",s+=1):a[c]?(m=10+(o[s+2]??0),f=10,l=i.rolls.map(d=>d.symbol).join(""),s+=2):(m=(o[s]??0)+(o[s+1]??0),f=m,l=i.rolls.map(d=>d.symbol).join(""),s+=2),n+=m,r.push({frameNumber:c+1,pinsKnocked:f,scoreContribution:m,cumulativeScore:n,rollSymbols:l,isStrike:i.isStrike,isSpare:i.isSpare})}return r}function ot(e){if(e.length!==10)return[];const o=e.slice(0,9),t=e[9],a=J(e),r=[];for(let s=0;s<9;s++){const n=o[s],c=a[s].scoreContribution,i=[];for(let d=0;d<9;d++){const p=[...o];[p[s],p[d]]=[p[d],p[s]];const $=[...p,t],w=J($)[d].scoreContribution;i.push(w)}const m=i.reduce((d,p)=>d+p,0)/i.length,f=c-m;let l="";if(n.isStrike)f>5?l=`Lucky placement! Strike got strong bonuses (${Math.round(f)} pins above average for this position)`:f<-5?l=`Unlucky placement! Strike got weak bonuses (${Math.round(Math.abs(f))} pins below average)`:l=`Strike in neutral position (within ${Math.round(Math.abs(f))} pins of average)`;else if(n.isSpare)f>3?l=`Great timing! Spare got a strong bonus ball (${Math.round(f)} pins above average)`:f<-3?l=`Bad timing! Spare got a weak bonus ball (${Math.round(Math.abs(f))} pins below average)`:l=`Spare in neutral position (within ${Math.round(Math.abs(f))} pins of average)`;else{const d=n.rolls.reduce((p,$)=>p+$.value,0);Math.abs(f)<=.5?l=`Open frame (${d} pins) - position doesn't matter much`:l=`Open frame (${d} pins) - minimal positional impact`}r.push({frameNumber:s+1,actualContribution:c,averageContribution:Math.round(m*10)/10,positionBenefit:Math.round(f*10)/10,rollSymbols:a[s].rollSymbols,isStrike:n.isStrike,isSpare:n.isSpare,explanation:l})}return r}const Me="bowling_fortune_saved_games",pe="bowling_fortune_draft",Se=1e4;function st(e,o,t,a){const r=oe(),s={id:lt(),scores:e,description:o,league:t,date:a||new Date().toISOString().split("T")[0],savedAt:Date.now(),gameCount:e.trim().split(`
`).filter(n=>n.trim()).length,totalScore:dt(e)};return r.games.unshift(s),r.games.length>Se&&(r.games=r.games.slice(0,Se)),se(r),s}function z(){return oe().games}function nt(e){const o=oe();o.games=o.games.filter(t=>t.id!==e),se(o)}function rt(){se({version:1,games:[]})}function at(){const e=z(),o=new Set;for(const t of e)t.league&&t.league.trim()&&o.add(t.league.trim());return Array.from(o).sort()}function it(){const e=oe();return JSON.stringify(e,null,2)}function ct(e){try{const o=JSON.parse(e);if(!o.version||!Array.isArray(o.games))return{success:!1,count:0,error:"Invalid file format"};for(const t of o.games)if(!t.id||!t.scores||typeof t.savedAt!="number")return{success:!1,count:0,error:"Invalid game data in file"};return se(o),{success:!0,count:o.games.length}}catch{return{success:!1,count:0,error:"Failed to parse JSON file"}}}function oe(){try{const e=localStorage.getItem(Me);return e?JSON.parse(e):{version:1,games:[]}}catch(e){return console.error("Failed to load saved games",e),{version:1,games:[]}}}function se(e){try{localStorage.setItem(Me,JSON.stringify(e))}catch(o){throw o instanceof DOMException&&o.name==="QuotaExceededError"&&alert("Storage quota exceeded. Please delete some saved games."),console.error("Failed to save games",o),o}}function lt(){return`${Date.now()}_${Math.random().toString(36).substr(2,4)}`}function dt(e){try{const o=e.trim().split(`
`).filter(a=>a.trim());let t=0;for(const a of o){const r=Le(a);if(r.kind==="error")return;t+=K(r.frames)}return t}catch{return}}function ut(e){try{e.trim()?localStorage.setItem(pe,e):he()}catch(o){console.error("Failed to save draft",o)}}function mt(){try{return localStorage.getItem(pe)}catch(e){return console.error("Failed to load draft",e),null}}function he(){try{localStorage.removeItem(pe)}catch(e){console.error("Failed to clear draft",e)}}function ft(e){const o=[],a=e.split(/<span class="title-headlines">Game \d+<\/span>/).slice(1);if(a.length===0)throw new Error("No games found in LaneTalk HTML");for(const s of a){const n=[],c=s.matchAll(/<div class="frame">.*?<div class="throws">(.*?)<\/div>\s*<div class="score">/gs);for(const m of c){const f=m[1],l=pt(f);n.push(l)}const i=s.match(/<div class="lastFrame">.*?<div class="throws">(.*?)<\/div>\s*<div class="score">/s);if(i){const m=i[1],f=ht(m);n.push(f)}if(n.length!==10)throw new Error(`Expected 10 frames, found ${n.length}`);o.push({frames:n.join(" ")})}const r=gt(e);return{games:o,metadata:r}}function pt(e){const o=e.matchAll(/font-size: 20px[^>]*>\s*([XxX\-0-9]+)\s*<\/span>/g),t=[];for(const r of o)t.push(r[1]);return e.includes('<div class="triangle"></div>')&&t.length===1&&t.push("/"),t.join("")}function ht(e){const o=e.matchAll(/font-size: 20px[^>]*>\s*([XxX\-0-9]+)\s*<\/span>/g),t=[];for(const r of o)t.push(r[1]);const a=(e.match(/<div class="triangle"><\/div>/g)||[]).length;for(let r=0;r<a;r++)t.push("/");return t.join("")}function gt(e){const o={},t=e.match(/<h1>([^<]+)<\/h1>/);t&&(o.bowler=t[1].trim());const a=e.match(/<h2>([^<]*(?:AM|PM)[^<]*)<\/h2>/);a&&(o.date=a[1].trim());const r=e.match(/<h2 class="name">([^<]+)<\/h2>/);r&&(o.location=r[1].trim());const s=e.match(/<span>Total<\/span>\s*<h2>(\d+)<\/h2>/s);s&&(o.total=parseInt(s[1],10));const n=e.match(/<span>Average<\/span>\s*<h2>(\d+)<\/h2>/s);return n&&(o.average=parseInt(n[1],10)),o}function vt(e){const o=/https?:\/\/shared\.lanetalk\.com\/[^\s]*/i,t=e.match(o);return t?t[0]:null}function bt(e){try{return new URL(e).hostname==="shared.lanetalk.com"}catch{return!1}}const $e=["Tell My Bowling Fortune","Glimpse Into My Future.. er, Past","Peer Into the Multiverse","Clutch Or Not?","My mom said I'm pretty good.","What oil pattern is this? Badger?","Calculate my shame","How lucky was I, really?","Did I deserve this score?","Explain my misery","Tell me I'm special","Judge my frame order","Was that skill or luck?","Is this thing scratch-and-sniff?","Like a 50/50 raffle, but you never win","We lost by 3 pins, but it definitely wasn't my fault... right?"],we=["Tell Me How Bad I Fucked Up","RATE MUH BALLS"],yt=.001,Te=[{name:"Perfect Game (300)",description:"The ultimate achievement - 12 strikes in a row",score:"X X X X X X X X X XXX"},{name:"Lucky Game",description:"Actual score much higher than median - very favorable frame order",score:"81 72 63 54 9/ X X X X XXX"},{name:"Unlucky Game",description:"Actual score lower than median - unfavorable frame order",score:"2/ 1/ 35 X 34 62 4/ 45 8/ 60"},{name:"Average Game",description:"Typical performance - score close to median",score:"9/ 45 03 7/ 40 90 09 9/ X 04"},{name:"Low Score Game",description:"Rough day at the lanes - lots of open frames",score:"52 7- 43 8- 61 72 54 6- 81 7-"},{name:"Multiple Games Series",description:"Three-game series showing different performances",score:`9/ X 81 7/ X X 9- 90 X XX6
...
7/ 6- X 81 9/ X 7- X X X90`},{name:"Clutch Performance",description:"Strong finish with strikes in the 10th",score:"7/ 8/ 81 9- 72 X 9/ 8- X XXX"},{name:"All Spares Game",description:"Consistent spare shooting - no strikes, no open frames",score:"9/ 8/ 7/ 6/ 5/ 4/ 3/ 2/ 1/ 9/9"}],Ee=document.querySelector("#app");if(!Ee)throw new Error("Failed to find app container");Ee.innerHTML=`
  ...
  <footer class="version">
    <p>Build: 2025-11-06 17:10:18 CT</p>
  </footer>
...
`;const y=document.querySelector("#scores-input"),U=document.querySelector("#submit"),Ce=document.querySelector("#clear-btn"),P=document.querySelector("#example-btn"),I=document.querySelector("#example-dropdown"),C=document.querySelector("#feedback"),W=document.querySelector("#lanetalk-url"),Z=document.querySelector("#lanetalk-import-btn"),Y=document.querySelector("#lanetalk-status"),Ae=document.querySelector("#help-icon"),B=document.querySelector("#scores-help"),ne=document.querySelector("#help-dialog-overlay"),Xe=document.querySelector("#help-close-btn"),Ie=document.querySelector("#save-btn"),qe=document.querySelector("#saved-games-btn"),Fe=document.querySelector("#saved-count"),N=document.querySelector("#save-modal-overlay"),De=document.querySelector("#save-form"),Q=document.querySelector("#save-description"),ge=document.querySelector("#save-league"),Be=document.querySelector("#league-list"),ee=document.querySelector("#save-date"),Ne=document.querySelector("#save-cancel-btn"),_=document.querySelector("#saved-games-sidebar"),re=document.querySelector("#sidebar-overlay"),Pe=document.querySelector("#sidebar-close-btn"),ae=document.querySelector("#search-saved-games"),Re=document.querySelector("#export-btn"),Ge=document.querySelector("#import-btn"),j=document.querySelector("#import-file-input"),Oe=document.querySelector("#clear-all-btn"),H=document.querySelector("#saved-games-list"),He=document.querySelector("#sidebar-saved-count");if(!y||!U||!Ce||!P||!I||!C||!W||!Z||!Y||!Ae||!B||!ne||!Xe||!Ie||!qe||!Fe||!N||!De||!Q||!ge||!Be||!ee||!Ne||!_||!re||!Pe||!ae||!Re||!Ge||!j||!Oe||!H||!He)throw new Error("Failed to initialise UI elements");Ce.addEventListener("click",()=>{y.value="",C.innerHTML="",he(),y.focus()});let le;function St(e){le!==void 0&&clearTimeout(le),le=window.setTimeout(()=>{ut(e)},500)}y.addEventListener("input",()=>{St(y.value)});let X=!1;function $t(){X=!X,I.classList.toggle("show",X),P.setAttribute("aria-expanded",X.toString()),I.setAttribute("aria-hidden",(!X).toString())}function ve(){X=!1,I.classList.remove("show"),P.setAttribute("aria-expanded","false"),I.setAttribute("aria-hidden","true")}P.addEventListener("click",e=>{e.stopPropagation(),$t()});const je=I.querySelectorAll(".dropdown-item");je.forEach(e=>{e.addEventListener("click",o=>{o.stopPropagation();const t=parseInt(e.getAttribute("data-example-index")||"0",10);y.value=Te[t].score,ve(),y.focus()})});document.addEventListener("click",e=>{const o=e.target;X&&!P.contains(o)&&!I.contains(o)&&ve()});I.addEventListener("keydown",e=>{if(e.key==="ArrowDown"||e.key==="ArrowUp"){e.preventDefault();const o=Array.from(je),t=o.indexOf(document.activeElement);let a;e.key==="ArrowDown"?a=t<o.length-1?t+1:0:a=t>0?t-1:o.length-1,o[a]?.focus()}});function wt(){B.classList.add("show"),ne.classList.add("show"),B.setAttribute("aria-hidden","false")}function be(){B.classList.remove("show"),ne.classList.remove("show"),B.setAttribute("aria-hidden","true")}Ae.addEventListener("click",e=>{e.preventDefault(),wt()});Xe.addEventListener("click",be);ne.addEventListener("click",be);function O(e,o){Y.textContent=e,Y.className=`lanetalk-status ${o}`}function kt(){Y.textContent="",Y.className="lanetalk-status"}async function Ue(){const e=W.value.trim();if(!e){O("Please enter a LaneTalk URL","error");return}const o=vt(e);if(!o||!bt(o)){O("Invalid LaneTalk URL. Must be from shared.lanetalk.com","error");return}Z.disabled=!0,O("Fetching games from LaneTalk...","loading");try{const t=`https://api.allorigins.win/raw?url=${encodeURIComponent(o)}`,a=await fetch(t);if(!a.ok)throw new Error(`Failed to fetch: ${a.status} ${a.statusText}`);const r=await a.text(),s=ft(r);if(s.games.length===0)throw new Error("No games found in LaneTalk data");const n=s.games.map(i=>i.frames).join(`
`);y.value=n;const c=s.games.length===1?"game":"games";O(`✓ Successfully imported ${s.games.length} ${c}!`,"success"),W.value="",setTimeout(()=>{kt(),y.focus()},2e3)}catch(t){console.error("LaneTalk import error:",t);const a=t instanceof Error?t.message:"Unknown error";O(`Error: ${a}`,"error")}finally{Z.disabled=!1}}Z.addEventListener("click",Ue);W.addEventListener("keydown",e=>{e.key==="Enter"&&(e.preventDefault(),Ue())});let de=0;function Ye(){if(Math.random()<yt){const o=Math.floor(Math.random()*we.length);U.textContent=we[o];return}U.textContent=$e[de],de=(de+1)%$e.length}Ye();setInterval(Ye,3e4);let ze="";function te(){if(!y.value.trim()){me("Please provide at least one game.",1,1);return}const e=y.value.replace(/\r/g,"").split(`
`),o=[];for(let t=0;t<e.length;t+=1){const a=e[t];if(!a.trim()){me(`Game ${t+1} is empty. Each line must contain exactly ten frames.`,t+1,1);return}const r=Le(a);if(r.kind==="error"){Et(r,t,e);return}const s=K(r.frames),n=tt(r.frames);o.push({frames:r.frames,score:s,stats:n})}ze=y.value,Pt(o)}U.addEventListener("click",te);y.addEventListener("keydown",e=>{e.key==="Enter"&&(e.ctrlKey||e.metaKey)&&(e.preventDefault(),te())});document.addEventListener("keydown",e=>{if(e.key==="Escape"){if(B.classList.contains("show")){be();return}if(_.classList.contains("show")){ce();return}if(N.classList.contains("show")){ie();return}if(X){ve(),P.focus();return}C.innerHTML&&(C.innerHTML="",y.focus())}});function V(){const o=z().length;Fe.innerHTML=o>0?`&nbsp;(${o})`:"",He.innerHTML=o>0?`&nbsp;(${o})`:""}function xt(){if(!y.value.trim()){L("Please enter some scores first");return}const e=new Date().toISOString().split("T")[0];ee.value=e,Q.value="",ge.value="";const o=at();Be.innerHTML=o.map(t=>`<option value="${t}">`).join(""),N.classList.add("show"),Q.focus()}function ie(){N.classList.remove("show")}function Lt(){ae.value="",R(),_.classList.add("show"),re.classList.add("show")}function ce(){_.classList.remove("show"),re.classList.remove("show")}function R(){const e=ae.value.trim().toLowerCase();let o=z();if(e&&(o=o.filter(t=>{const a=(t.description||"").toLowerCase(),r=(t.league||"").toLowerCase();return a.includes(e)||r.includes(e)})),o.length===0){H.innerHTML=e?'<p class="empty-state">No games match your search.</p>':'<p class="empty-state">No saved games yet. Save your first game!</p>';return}H.innerHTML=o.map(t=>{const a=t.gameCount===1?"1 game":`${t.gameCount} games`,r=t.totalScore!==void 0?`🎯 ${t.totalScore}`:"⚠️ Invalid",s=t.description||"(No description)",n=t.league?`🏆 ${t.league}`:"";return`
    ...
    `}).join(""),H.querySelectorAll("[data-load-id]").forEach(t=>{t.addEventListener("click",()=>{const a=t.getAttribute("data-load-id");a&&Mt(a)})}),H.querySelectorAll("[data-delete-id]").forEach(t=>{t.addEventListener("click",()=>{const a=t.getAttribute("data-delete-id");a&&Tt(a)})})}function Mt(e){const t=z().find(a=>a.id===e);t&&(y.value=t.scores,ce(),U.click(),L("Game loaded!"))}function Tt(e){confirm("Delete this saved game?")&&(nt(e),V(),R(),L("Game deleted"))}Ie.addEventListener("click",xt);qe.addEventListener("click",Lt);Ne.addEventListener("click",ie);Pe.addEventListener("click",ce);re.addEventListener("click",ce);ae.addEventListener("input",()=>{R()});N.addEventListener("click",e=>{e.target===N&&ie()});De.addEventListener("submit",e=>{e.preventDefault();const o=y.value.trim(),t=Q.value.trim()||void 0,a=ge.value.trim()||void 0;let r=ee.value||void 0;if(r){const s=new Date(r),n=new Date;if(n.setHours(0,0,0,0),s>n){L("Date cannot be in the future"),ee.focus();return}}try{st(o,t,a,r),ie(),he(),V(),_.classList.contains("show")&&R(),L("Game saved!")}catch(s){console.error("Failed to save game",s),L("Failed to save game")}});Oe.addEventListener("click",()=>{confirm("Delete ALL saved games? This cannot be undone.")&&(rt(),V(),R(),L("All games deleted"))});Re.addEventListener("click",()=>{const e=z();if(e.length===0){L("No games to export");return}const o=it(),t=new Blob([o],{type:"application/json"}),a=URL.createObjectURL(t),r=document.createElement("a");r.href=a,r.download=`bowling-games-${new Date().toISOString().split("T")[0]}.json`,document.body.appendChild(r),r.click(),document.body.removeChild(r),URL.revokeObjectURL(a),L(`Exported ${e.length} game${e.length===1?"":"s"}`)});Ge.addEventListener("click",()=>{j.click()});j.addEventListener("change",e=>{const o=e.target.files?.[0];if(!o)return;const t=new FileReader;t.onload=a=>{const r=a.target?.result,s=ct(r);s.success?(V(),R(),L(`Imported ${s.count} game${s.count===1?"":"s"}`)):L(s.error||"Import failed"),j.value=""},t.onerror=()=>{L("Failed to read file"),j.value=""},t.readAsText(o)});V();window.addEventListener("DOMContentLoaded",()=>{const o=new URLSearchParams(window.location.search).get("scores");if(o)try{const t=atob(o);y.value=t,te()}catch(t){console.error("Failed to decode scores from URL",t)}else{const t=mt();t&&(y.value=t,te())}});function Et(e,o,t){const a=o+1,r=`Row ${a}, column ${e.column}: ${e.message}`,s=_e(t,o,e.column);me(r,a,e.column,s)}function _e(e,o,t){let a=0;for(let r=0;r<o;r+=1)a+=e[r].length+1;return a+(t-1)}function me(e,o,t,a){if(C.innerHTML="",C.className="error",C.textContent=e,y.focus(),typeof a=="number")y.setSelectionRange(a,a);else{const r=y.value.replace(/\r/g,"").split(`
`),s=_e(r,o-1,t);y.setSelectionRange(s,s)}}function Ct(e){const{histogram:o,median:t}=e.stats,a=e.score,r=600,s=300,n={top:20,right:20,bottom:40,left:50},c=r-n.left-n.right,i=s-n.top-n.bottom,m=e.stats.min,f=e.stats.max,l=new Map(o.map(b=>[b.score,b])),d=[];for(let b=m;b<=f;b++){const v=l.get(b);d.push({score:b,count:v?.count??0,frequency:v?.frequency??0})}const p=Math.max(...d.map(b=>b.count)),$=Math.max(2,c/d.length),k=d.map((b,v)=>{const u=n.left+v*c/d.length,g=b.count/p*i,G=n.top+i-g,A=b.score===a;return`<rect
      x="${u}"
      y="${G}"
      width="${$}"
      height="${g}"
...
Frequency: ${(b.frequency*100).toFixed(2)}%</title>
    </rect>`}).join(""),w=t-m,x=n.left+w*c/d.length+$/2,q=`
  ...
  `,E=5,T=Array.from({length:E+1},(b,v)=>{const u=Math.round(p/E*v),g=n.top+i-v*i/E;return`
...
Combinations: ${u.count.toLocaleString()}</title>
    </rect>`}).join(""),E=w-m,T=n.left+E*c/d.length+x/2,h=`
    <line x1="${T}" y1="${n.top}" x2="${T}" y2="${n.top+i}"
          stroke="#ec4899" stroke-width="2" stroke-dasharray="5,5" />
  ...
  `,S="",b=Math.min(10,Math.ceil((f-m)/20)),v=b===0?`
      <line x1="${n.left}" y1="${n.top+i}" x2="${n.left}" y2="${n.top+i+5}" stroke="#94a3b8" stroke-width="1" />
      <text x="${n.left}" y="${n.top+i+20}" text-anchor="middle" font-size="11" fill="#94a3b8">${m}</text>
    `:Array.from({length:b+1},(u,g)=>{const G=Math.round(m+(f-m)/b*g),A=n.left+g*c/b;return`
        <line x1="${A}" y1="${n.top+i}" x2="${A}" y2="${n.top+i+5}" stroke="#94a3b8" stroke-width="1" />
        <text x="${A}" y="${n.top+i+20}" text-anchor="middle" font-size="11" fill="#94a3b8">${G}</text>
      `}).join("");return`
    <svg viewBox="0 0 ${r} ${s}" class="histogram">
      <rect x="0" y="0" width="${r}" height="${s}" fill="rgba(15, 23, 42, 0.5)" />
      ${q}
  ...
  `}function Ft(){const e=btoa(ze),o=new URL(window.location.href);return o.search=`?scores=${encodeURIComponent(e)}`,o.toString()}function Dt(){const e=Ft();navigator.clipboard.writeText(e).then(()=>{L("Link copied!")}).catch(o=>{console.error("Failed to copy link",o),L("Failed to copy link")})}function L(e){const o=document.querySelector(".toast");o&&o.remove();const t=document.createElement("div");t.className="toast",t.textContent=e,document.body.appendChild(t),setTimeout(()=>{t.classList.add("show")},10),setTimeout(()=>{t.classList.remove("show"),setTimeout(()=>{t.remove()},300)},2e3)}function Bt(e){return`
  ...
  `}function Nt(e){const o=J(e),t=ot(e),a=3,r=-3,s=t.filter(l=>l.positionBenefit>=a).sort((l,d)=>d.positionBenefit-l.positionBenefit),n=t.filter(l=>l.positionBenefit<=r).sort((l,d)=>l.positionBenefit-d.positionBenefit);function c(){return`
      <div class="complete-scorecard">
        <div class="scorecard-row">
  ...
  `}function Pt(e){if(C.className="output",e.length===0){C.innerHTML="";return}const o=`
    <button type="button" class="copy-link-btn" data-copy-link>
      Copy link 🔗
    </button>
  `,t=e.map((s,n)=>{const c=n+1,i=s.stats.mode.length===1?s.stats.mode[0].toString():`${s.stats.mode.join(", ")} (multimodal)`,m=s.score-s.stats.median,f=m>=0?`+${m}`:`${m}`,l=Xt(s);return`
  ...
  `,C.querySelectorAll("[data-copy-link]").forEach(s=>{s.addEventListener("click",Dt)})}
