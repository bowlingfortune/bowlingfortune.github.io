...
7/ 6- X 81 9/ X 7- X X X90`},{name:"Clutch Performance",description:"Strong finish with strikes in the 10th",score:"7/ 8/ 81 9- 72 X 9/ 8- X XXX"},{name:"All Spares Game",description:"Consistent spare shooting - no strikes, no open frames",score:"9/ 8/ 7/ 6/ 5/ 4/ 3/ 2/ 1/ 9/9"}],Ee=document.querySelector("#app");if(!Ee)throw new Error("Failed to find app container");Ee.innerHTML=`
  ...
  <footer class="version">
    <p>Build: 2025-11-06 17:00:30 CT</p>
  </footer>
...
`),t=[];for(let o=0;o<e.length;o+=1){const a=e[o];if(!a.trim()){me(`Game ${o+1} is empty. Each line must contain exactly ten frames.`,o+1,1);return}const r=Le(a);if(r.kind==="error"){Tt(r,o,e);return}const s=K(r.frames),n=tt(r.frames);t.push({frames:r.frames,score:s,stats:n})}ze=y.value,Nt(t)}U.addEventListener("click",te);y.addEventListener("keydown",e=>{e.key==="Enter"&&(e.ctrlKey||e.metaKey)&&(e.preventDefault(),te())});document.addEventListener("keydown",e=>{if(e.key==="Escape"){if(B.classList.contains("show")){be();return}if(_.classList.contains("show")){ce();return}if(N.classList.contains("show")){ie();return}if(X){ve(),P.focus();return}C.innerHTML&&(C.innerHTML="",y.focus())}});function V(){const t=z().length;Fe.innerHTML=t>0?`&nbsp;(${t})`:"",He.innerHTML=t>0?`&nbsp;(${t})`:""}function kt(){if(!y.value.trim()){L("Please enter some scores first");return}const e=new Date().toISOString().split("T")[0];ee.value=e,Q.value="",ge.value="";const t=at();Be.innerHTML=t.map(o=>`<option value="${o}">`).join(""),N.classList.add("show"),Q.focus()}function ie(){N.classList.remove("show")}function xt(){ae.value="",G(),_.classList.add("show"),re.classList.add("show")}function ce(){_.classList.remove("show"),re.classList.remove("show")}function G(){const e=ae.value.trim().toLowerCase();let t=z();if(e&&(t=t.filter(o=>{const a=(o.description||"").toLowerCase(),r=(o.league||"").toLowerCase();return a.includes(e)||r.includes(e)})),t.length===0){H.innerHTML=e?'<p class="empty-state">No games match your search.</p>':'<p class="empty-state">No saved games yet. Save your first game!</p>';return}H.innerHTML=t.map(o=>{const a=o.gameCount===1?"1 game":`${o.gameCount} games`,r=o.totalScore!==void 0?`🎯 ${o.totalScore}`:"⚠️ Invalid",s=o.description||"(No description)",n=o.league?`🏆 ${o.league}`:"";return`
    ...
    `}).join(""),H.querySelectorAll("[data-load-id]").forEach(o=>{o.addEventListener("click",()=>{const a=o.getAttribute("data-load-id");a&&Lt(a)})}),H.querySelectorAll("[data-delete-id]").forEach(o=>{o.addEventListener("click",()=>{const a=o.getAttribute("data-delete-id");a&&Mt(a)})})}function Lt(e){const o=z().find(a=>a.id===e);o&&(y.value=o.scores,ce(),U.click(),L("Game loaded!"))}function Mt(e){confirm("Delete this saved game?")&&(nt(e),V(),G(),L("Game deleted"))}Ie.addEventListener("click",kt);qe.addEventListener("click",xt);Ne.addEventListener("click",ie);Pe.addEventListener("click",ce);re.addEventListener("click",ce);ae.addEventListener("input",()=>{G()});N.addEventListener("click",e=>{e.target===N&&ie()});De.addEventListener("submit",e=>{e.preventDefault();const t=y.value.trim(),o=Q.value.trim()||void 0,a=ge.value.trim()||void 0;let r=ee.value||void 0;if(r){const s=new Date(r),n=new Date;if(n.setHours(0,0,0,0),s>n){L("Date cannot be in the future"),ee.focus();return}}try{st(t,o,a,r),ie(),he(),V(),_.classList.contains("show")&&G(),L("Game saved!")}catch(s){console.error("Failed to save game",s),L("Failed to save game")}});Re.addEventListener("click",()=>{confirm("Delete ALL saved games? This cannot be undone.")&&(rt(),V(),G(),L("All games deleted"))});Ge.addEventListener("click",()=>{const e=z();if(e.length===0){L("No games to export");return}const t=it(),o=new Blob([t],{type:"application/json"}),a=URL.createObjectURL(o),r=document.createElement("a");r.href=a,r.download=`bowling-games-${new Date().toISOString().split("T")[0]}.json`,document.body.appendChild(r),r.click(),document.body.removeChild(r),URL.revokeObjectURL(a),L(`Exported ${e.length} game${e.length===1?"":"s"}`)});Oe.addEventListener("click",()=>{j.click()});j.addEventListener("change",e=>{const t=e.target.files?.[0];if(!t)return;const o=new FileReader;o.onload=a=>{const r=a.target?.result,s=ct(r);s.success?(V(),G(),L(`Imported ${s.count} game${s.count===1?"":"s"}`)):L(s.error||"Import failed"),j.value=""},o.onerror=()=>{L("Failed to read file"),j.value=""},o.readAsText(t)});V();window.addEventListener("DOMContentLoaded",()=>{const t=new URLSearchParams(window.location.search).get("scores");if(t)try{const o=atob(t);y.value=o,te()}catch(o){console.error("Failed to decode scores from URL",o)}else{const o=mt();o&&(y.value=o,te())}});function Tt(e,t,o){const a=t+1,r=`Row ${a}, column ${e.column}: ${e.message}`,s=_e(o,t,e.column);me(r,a,e.column,s)}function _e(e,t,o){let a=0;for(let r=0;r<t;r+=1)a+=e[r].length+1;return a+(o-1)}function me(e,t,o,a){if(C.innerHTML="",C.className="error",C.textContent=e,y.focus(),typeof a=="number")y.setSelectionRange(a,a);else{const r=y.value.replace(/\r/g,"").split(`
`),s=_e(r,t-1,o);y.setSelectionRange(s,s)}}function Et(e){const{histogram:t,median:o}=e.stats,a=e.score,r=600,s=300,n={top:20,right:20,bottom:40,left:50},c=r-n.left-n.right,i=s-n.top-n.bottom,m=e.stats.min,f=e.stats.max,l=new Map(t.map(b=>[b.score,b])),d=[];for(let b=m;b<=f;b++){const v=l.get(b);d.push({score:b,count:v?.count??0,frequency:v?.frequency??0})}const p=Math.max(...d.map(b=>b.count)),$=Math.max(2,c/d.length),k=d.map((b,v)=>{const u=n.left+v*c/d.length,g=b.count/p*i,O=n.top+i-g,A=b.score===a;return`<rect
      x="${u}"
      y="${O}"
      width="${$}"
      height="${g}"
...
Frequency: ${(b.frequency*100).toFixed(2)}%</title>
    </rect>`}).join(""),w=o-m,x=n.left+w*c/d.length+$/2,q=`
...
Combinations: ${u.count.toLocaleString()}</title>
  ...
  `,S="",b=Math.min(10,Math.ceil((f-m)/20)),v=b===0?`
      <line x1="${n.left}" y1="${n.top+i}" x2="${n.left}" y2="${n.top+i+5}" stroke="#94a3b8" stroke-width="1" />
      <text x="${n.left}" y="${n.top+i+20}" text-anchor="middle" font-size="11" fill="#94a3b8">${m}</text>
    `:Array.from({length:b+1},(u,g)=>{const O=Math.round(m+(f-m)/b*g),A=n.left+g*c/b;return`
        <line x1="${A}" y1="${n.top+i}" x2="${A}" y2="${n.top+i+5}" stroke="#94a3b8" stroke-width="1" />
        <text x="${A}" y="${n.top+i+20}" text-anchor="middle" font-size="11" fill="#94a3b8">${O}</text>
      `}).join("");return`
    <svg viewBox="0 0 ${r} ${s}" class="histogram">
      <rect x="0" y="0" width="${r}" height="${s}" fill="rgba(15, 23, 42, 0.5)" />
      ${q}
  ...
  `}function Bt(e){const t=J(e),o=ot(e),a=3,r=-3,s=o.filter(l=>l.positionBenefit>=a).sort((l,d)=>d.positionBenefit-l.positionBenefit),n=o.filter(l=>l.positionBenefit<=r).sort((l,d)=>l.positionBenefit-d.positionBenefit);function c(){return`
      <div class="complete-scorecard">
        <div class="scorecard-row">
  ...
  `,o=e.map((s,n)=>{const c=n+1,i=s.stats.mode.length===1?s.stats.mode[0].toString():`${s.stats.mode.join(", ")} (multimodal)`,m=s.score-s.stats.median,f=m>=0?`+${m}`:`${m}`,l=At(s);return`
