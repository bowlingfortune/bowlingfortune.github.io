...
7/ 6- X 81 9/ X 7- X X X90`},{name:"Clutch Performance",description:"Strong finish with strikes in the 10th",score:"7/ 8/ 81 9- 72 X 9/ 8- X XXX"},{name:"All Spares Game",description:"Consistent spare shooting - no strikes, no open frames",score:"9/ 8/ 7/ 6/ 5/ 4/ 3/ 2/ 1/ 9/9"}],ve=document.querySelector("#app");if(!ve)throw new Error("Failed to find app container");ve.innerHTML=`
  ...
  <footer class="version">
    <p>Build: 2025-10-18 07:14:54 CT</p>
  </footer>
...
`),t=[];for(let o=0;o<e.length;o+=1){const a=e[o];if(!a.trim()){se(`Game ${o+1} is empty. Each line must contain exactly ten frames.`,o+1,1);return}const r=pe(a);if(r.kind==="error"){st(r,o,e);return}const n=U(r.frames),s=Ne(r.frames);t.push({frames:r.frames,score:n,stats:s})}Ie=$.value,mt(t)}z.addEventListener("click",ie);$.addEventListener("keydown",e=>{e.key==="Enter"&&(e.ctrlKey||e.metaKey)&&(e.preventDefault(),ie())});document.addEventListener("keydown",e=>{if(e.key==="Escape"){if(H.classList.contains("show")){ee();return}if(P.classList.contains("show")){Q();return}if(q){ae(),O.focus();return}A.innerHTML&&(A.innerHTML="",$.focus())}});function Y(){const t=j().length;Se.innerHTML=t>0?`&nbsp;(${t})`:"",Ae.innerHTML=t>0?`&nbsp;(${t})`:""}function Qe(){if(!$.value.trim()){x("Please enter some scores first");return}const e=new Date().toISOString().split("T")[0];_.value=e,V.value="",re.value="";const t=ze();xe.innerHTML=t.map(o=>`<option value="${o}">`).join(""),P.classList.add("show"),V.focus()}function Q(){P.classList.remove("show")}function et(){Z.value="",G(),H.classList.add("show"),K.classList.add("show")}function ee(){H.classList.remove("show"),K.classList.remove("show")}function G(){const e=Z.value.trim().toLowerCase();let t=j();if(e&&(t=t.filter(o=>{const a=(o.description||"").toLowerCase(),r=(o.league||"").toLowerCase();return a.includes(e)||r.includes(e)})),t.length===0){R.innerHTML=e?'<p class="empty-state">No games match your search.</p>':'<p class="empty-state">No saved games yet. Save your first game!</p>';return}R.innerHTML=t.map(o=>{const a=o.gameCount===1?"1 game":`${o.gameCount} games`,r=o.totalScore!==void 0?`🎯 ${o.totalScore}`:"⚠️ Invalid",n=o.description||"(No description)",s=o.league?`🏆 ${o.league}`:"";return`
    ...
    `}).join(""),R.querySelectorAll("[data-load-id]").forEach(o=>{o.addEventListener("click",()=>{const a=o.getAttribute("data-load-id");a&&tt(a)})}),R.querySelectorAll("[data-delete-id]").forEach(o=>{o.addEventListener("click",()=>{const a=o.getAttribute("data-delete-id");a&&ot(a)})})}function tt(e){const o=j().find(a=>a.id===e);o&&($.value=o.scores,ee(),$.focus(),x("Game loaded!"))}function ot(e){confirm("Delete this saved game?")&&(Ye(e),Y(),G(),x("Game deleted"))}ye.addEventListener("click",Qe);$e.addEventListener("click",et);ke.addEventListener("click",Q);Le.addEventListener("click",ee);K.addEventListener("click",ee);Z.addEventListener("input",()=>{G()});P.addEventListener("click",e=>{e.target===P&&Q()});we.addEventListener("submit",e=>{e.preventDefault();const t=$.value.trim(),o=V.value.trim()||void 0,a=re.value.trim()||void 0;let r=_.value||void 0;if(r){const n=new Date(r),s=new Date;if(s.setHours(0,0,0,0),n>s){x("Date cannot be in the future"),_.focus();return}}try{He(t,o,a,r),Q(),Y(),H.classList.contains("show")&&G(),x("Game saved!")}catch(n){console.error("Failed to save game",n),x("Failed to save game")}});Xe.addEventListener("click",()=>{confirm("Delete ALL saved games? This cannot be undone.")&&(Ue(),Y(),G(),x("All games deleted"))});Me.addEventListener("click",()=>{const e=j();if(e.length===0){x("No games to export");return}const t=Ve(),o=new Blob([t],{type:"application/json"}),a=URL.createObjectURL(o),r=document.createElement("a");r.href=a,r.download=`bowling-games-${new Date().toISOString().split("T")[0]}.json`,document.body.appendChild(r),r.click(),document.body.removeChild(r),URL.revokeObjectURL(a),x(`Exported ${e.length} game${e.length===1?"":"s"}`)});Ce.addEventListener("click",()=>{N.click()});N.addEventListener("change",e=>{const t=e.target.files?.[0];if(!t)return;const o=new FileReader;o.onload=a=>{const r=a.target?.result,n=_e(r);n.success?(Y(),G(),x(`Imported ${n.count} game${n.count===1?"":"s"}`)):x(n.error||"Import failed"),N.value=""},o.onerror=()=>{x("Failed to read file"),N.value=""},o.readAsText(t)});Y();window.addEventListener("DOMContentLoaded",()=>{const t=new URLSearchParams(window.location.search).get("scores");if(t)try{const o=atob(t);$.value=o,ie()}catch(o){console.error("Failed to decode scores from URL",o)}});function st(e,t,o){const a=t+1,r=`Row ${a}, column ${e.column}: ${e.message}`,n=Te(o,t,e.column);se(r,a,e.column,n)}function Te(e,t,o){let a=0;for(let r=0;r<t;r+=1)a+=e[r].length+1;return a+(o-1)}function se(e,t,o,a){if(A.innerHTML="",A.className="error",A.textContent=e,$.focus(),typeof a=="number")$.setSelectionRange(a,a);else{const r=$.value.replace(/\r/g,"").split(`
